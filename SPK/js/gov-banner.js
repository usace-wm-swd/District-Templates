...
function createScrollListener() {
    ...
    banner_scroll_px = window.innerHeight * SCROLL_AMOUNT_PCT
    // Set a minimum threshold of 25 pixels
    scroll_banner_threshold_px = banner_scroll_px < 25 ? 25 : banner_scroll_px
...
// When the user scrolls, hide the Gov Banner at 10% the screen height
let govBannerScrollEvent = createScrollListener()

// Let the user click anywhere inside the banner to expand
GOV_BANNER.addEventListener("click", (e)=> {
    e.preventDefault()
...
GOV_BANNER_EXPAND.addEventListener("keypress", (e)=> {
    if (e.key === "Enter")
        e.target.click()
})
